...
public:
 BoundingBox() { }
 BoundingBox(const Vec3d &_min, const Vec3d &_max) : min(_min), max(_max) { }
	Vec3d min;
	Vec3d max;
...
public:
	Scene() 
	  : transformRoot(), objects(), lights(), hbv(NULL)
		{}
	virtual ~Scene();
	...
	void add( Geometry* obj ) {
		...
		bool hasBBox;
		if((hasBBox = obj->hasBoundingBoxCapability()) && objects.size() == 0) {
		  sceneBounds = obj->getBoundingBox();
		} else if(hasBBox) {
		  const BoundingBox &bbox = obj->getBoundingBox();
		  sceneBounds.min = minimum(sceneBounds.min, bbox.min);
		  sceneBounds.max = maximum(sceneBounds.max, bbox.max);
		}
		if(hasBBox) {
